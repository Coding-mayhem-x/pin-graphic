...
class Honeycomb {
  ...
  addCAPaletteStep(palette: ColorEntry[], selectedId?: string): boolean {
    ...
    for (const comp of comps) {
      ...
      const uniq: Axial[] = []; const seen = new Set<string>();
  ...
  addCAColorStep(ruleKey: string = "majority2"): boolean {
    ...
    for (const k of this.frontier) {
      ...
      if (!birthColor) continue;
      // score: prefer more neighbors and stronger majority
      let maxSame = 0; for (const v of counts.values()) { if (v > maxSame) maxSame = v; }
      const score = total * 10 + maxSame;
      if (!best || score > best.score) { best = { a, score, color: birthColor }; }
    }
    if (best) {
      const b = best.a; const k = this.key(b); this.frontier.delete(k);
      const p = this.axialToPoint(b);
      if (!this.withinArea(p)) return false;
      this.place(b, best.color);
      this.renderCircle(b, p);
  ...
  applyDitherMixFromSampler(s: any, fit: 'cover'|'contain'|'fill', palette: ColorEntry[]) {
    if (!s || !s.isReady || !s.isReady()) { return; }
...
function main() {
  ...
  (document.getElementById('btnAddRandomColor') as HTMLButtonElement).onclick = () => {
    ...
    else if (strat === 'clock2') { const all = palette.colors; if (!all.length) return; const pick = all[Math.floor(Math.random() * all.length)].value; model.addClockV2WithColor(pick); } else if (strat === 'clock3') {} else if (strat === 'ca-color') {} else if (strat === 'ca-palette') { (model as any).addCAPaletteStep(palette.colors, (palette.selected && palette.selected.id) || undefined); } else { const sel = palette.selected; if (!sel) return; model.addRandomWithColor(sel.value); }
  };
...
function nearestTwoFromPalette(rgb: RGB, palette: ColorEntry[]): {a:string,b:string, da:number, db:number} {
  ...
  if (bestB === bestA && palette.length>1){ const alt = palette.find(x=>x.value!==bestA); if(alt){ bestB = alt.value; db = rgbDist2(rgb, parseCssColorToRgb(bestB)!); } }
