...
(function () {
    const ST = window.CA_V2 || (window.CA_V2 = { state: { colorOrder: [], colorPtr: 0, compPtr: {} } });
    function M() { return window.honeyModel; }
    function key(a) { const m = M(); return m.key(a); }
    function within(a) { const m = M(); return m.withinArea(m.axialToPoint(a)); }
    ...
    function refreshOrder() {
        const st = ST.state;
        const now = colorsPresent();
        st.colorOrder = now;
        if (st.colorPtr >= now.length)
            st.colorPtr = 0; // trim
        // keep comp pointers per color
        const cur = st.compPtr;
        const next = {};
        for (const c of now) {
            next[c] = (cur[c] || 0);
        }
    ...
    function compsOf(col) { const m = M(); try {
        return m.componentsOfColor ? m.componentsOfColor(col) : [];
    }
    catch {
        return [];
    } }
    function freeNeighbors(comp) { const m = M(); try {
        return m.freeNeighbors ? m.freeNeighbors(comp) : [];
    }
    catch {
        return [];
    } }
    function neighCounts(a) { const m = M(); const map = new Map(); for (const n of m.neighbors(a)) {
        const nk = key(n);
        if (m.placed.has(nk)) {
            const c = m.colorByKey.get(nk);
            if (c) {
                map.set(c, (map.get(c) || 0) + 1);
            }
        }
    } return map; }
    function pickForComp(col, comp) {
    ...
    function step() {
        ...
        for (const c of colors) {
            const comps = compsOf(c).filter(x => x && x.length);
            compsBy[c] = comps;
            total += comps.length;
            st.compPtr[c] = st.compPtr[c] || 0;
        }
        if (total === 0)
            return false;
        for (let tries = 0; tries < total; tries++) {
            const c = colors[st.colorPtr % colors.length];
            const comps = compsBy[c];
            st.colorPtr = (st.colorPtr + 1) % colors.length;
            if (!comps.length)
                continue;
            const i = st.compPtr[c] % comps.length;
            st.compPtr[c] = (st.compPtr[c] + 1) % Math.max(1, comps.length);
            const comp = comps[i];
            const pick = pickForComp(c, comp);
            if (pick) {
                return place(pick);
            }
        }
    ...
    function wire() {
        ...
        if (any) {
            any.addEventListener('click', function (ev) { const cur = (sel.value || ''); if (cur === 'ca-v2') {
